...
describe "Costume" do
  ...
  it "knows when it was created" do
    Costume.create(
      name: "Dog's Bumble Bee",
      image_url: "http://cdn.sheknows.com/filter/l/gallery/halloween_costume_dog_bumblebee.jpg",
      size: "medium"
    )
    bee = Costume.find_by(name: "Dog's Bumble Bee")
    expect { bee.created_at }.to_not raise_error
    expect(bee.created_at.to_datetime === Date.today).to eq(true)
  end
  ...
  it "knows when it was last updated" do
    ...
    expect(stegosaurus.updated_at.to_datetime === Date.today).to eq(true)
